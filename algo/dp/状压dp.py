# 状态dp:
# 暴力方法是枚举所有排列，计算每个排列的得分，复杂度为O(n * n!)，可解决 n <= 10 的问题
# 状压dp可以把时间压缩到 O(n * 2^n)，可解决 n <= 20 的问题
# 1. 排列无关型: f[S] 定义已经排好的元素或可以选元素集合为S时的最优值。通过枚举当前要填的元素来转移。
# 2. 排列有关型：f[S][i] 定义未选（或已选）的集合为S, 且上一个元素为i时的最优值。通过枚举当前要填的元素来转移。
